...
const TextBubble := preload("res://Resources/Dialouges/Dialogic/Styles/custom/text_bubble.gd")

var bubbles: Array[TextBubble] = []
var fallback_bubble: TextBubble = null

const textbubble_scene: PackedScene = preload("res://Resources/Dialouges/Dialogic/Styles/custom/text_bubble.tscn")
...
func bubble_apply_overrides(bubble:TextBubble) -> void:
	...
	## CHOICE SETTINGS
	# Create choice container only once
	if bubble.choice_container == null:

		if choices_layout_force_lines:
			bubble.add_choice_container(
				VBoxContainer.new(),
				choices_layout_alignment,
				choices_custom_button,
				maximum_choices
			)
		else:
			bubble.add_choice_container(
	...
	if choices_base_theme.is_empty() or not ResourceLoader.exists(choices_base_theme):
		...
		choice_theme = load(choices_base_theme).duplicate()
	...
	choice_theme.set_color(
		&"font_hover_color",
		&"Button",
		choices_text_color_hover
	)

	choice_theme.set_color(
		&"font_focus_color",
		&"Button",
		choices_text_color_focus
	)

	choice_theme.set_color(
		&"font_disabled_color",
		&"Button",
		choices_text_color_disabled
	)
